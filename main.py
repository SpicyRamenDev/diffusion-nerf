...
def parse_args():
    ...
    nef_group.add_argument('--pos-embedder', type=str, choices=['none', 'identity', 'positional'],
                           ...
                           help='MLP Decoder of neural field: Positional embedder used to encode input coordinates'
                                'or view directions.')
    nef_group.add_argument('--view-embedder', type=str, choices=['none', 'identity', 'positional', 'spherical'],
                           default='positional',
                           help='MLP Decoder of neural field: View embedder used to encode view direction')
    ...
    optimizer_group.add_argument('--orientation-loss', type=float, default=0.1,
                                 help='Orientation regularization loss multiplier.')
    optimizer_group.add_argument('--opacity-loss', type=float, default=0.001,
                                 help='Opacity loss multiplier.')
    optimizer_group.add_argument('--entropy-loss', type=float, default=0.002,
                                 help='Entropy loss multiplier.')
    optimizer_group.add_argument('--total-variation-loss', type=float, default=0.002,
                                 help='Total variation loss multiplier.')
    optimizer_group.add_argument('--diffusion-loss', type=float, default=1.0,
                                 help='Diffusion loss multiplier.')
    optimizer_group.add_argument('--albedo-steps', type=int, default=1000,
                                 help='Number of albedo steps.')
    optimizer_group.add_argument('--iterations-per-epoch', type=int, default=100,
                                 help='Iterations per epoch.')
    optimizer_group.add_argument('--warmup-iterations', type=int, default=0,
                                 help='Warmup iterations.')
    optimizer_group.add_argument('--init-lr', type=float, default=1e-2,
                                 help='Init iterations.')
    optimizer_group.add_argument('--end-lr', type=float, default=1e-1,
                                 help='End iterations.')
    optimizer_group.add_argument('--reg-warmup-iterations', type=int, default=0,
                                 help='Reg warmup iterations.')
    optimizer_group.add_argument('--reg-init-lr', type=float, default=1e-2,
                                 help='Reg init iterations.')
    optimizer_group.add_argument('--phase-ratios', nargs=2, type=float, default=[0.1, 0.7],
                                 help='Iterations ratios of the phases.')
    optimizer_group.add_argument('--minibatch-size', type=int, default=1,
                                 help='Minibatch size.')
    optimizer_group.add_argument('--my-bg-color', type=str, default='decoder',
                                 help='My bg color.')

    nef_group.add_argument('--bottleneck-dim', type=int, default=8,
                           help='Dimension of the bottleneck features.')
    nef_group.add_argument('--blob-scale', type=float, default=5.0,
    ...
    diffusion_group.add_argument('--repo-id', type=str, default='stabilityai/stable-diffusion-2-1-base',
                                 help='The repo id for the stable diffusion model to use.')
    diffusion_group.add_argument('--prompt', type=str, default='a 3D render of an apple',
    ...
    diffusion_group.add_argument('--use-view-prompt', action='store_true', default=True,
                                 help='Use view dependent prompt.')
    diffusion_group.add_argument('--no-view-prompt', action='store_false', dest='use_view_prompt',
                                 help='Do not use view dependent prompt.')

    camera_group = parser.add_argument_group('camera')
    camera_group.add_argument('--azimuth-range', nargs=2, type=float, default=[0, 360],
                              help='Range of the azimuth angle of the camera.')
    camera_group.add_argument('--polar-range', nargs=2, type=float, default=[0, 100],
                              help='Range of the polar angle of the camera.')
    camera_group.add_argument('--camera-distance-range', nargs=2, type=float, default=[3.5, 4.5],
                              help='Range of the camera distance.')
    camera_group.add_argument('--focal-length-multiplier-range', nargs=2, type=float, default=[0.7, 1.35],
                              help='Range of the focal length multiplier.')
    camera_group.add_argument('--camera-offset', type=float, default=0.1,
                              help='Range of the offset of the camera from its position.')
    camera_group.add_argument('--camera-up-std', type=float, default=0.02,
                              help='Std of the camera up vector.')
    camera_group.add_argument('--look-at-std', type=float, default=0.2,
                              help='Std of the look up vector.')
    camera_group.add_argument('--light-distance-range', nargs=2, type=float, default=[0.8, 1.5],
                              help='Range of the light distance.')
    camera_group.add_argument('--resolutions', nargs=3, type=int, default=[64, 64, 64],
                              help='Resolutions for each phases.')
...
def load_dataset(args) -> torch.utils.data.Dataset:
    ...
    transform = SampleNoisyRays(num_samples=args.num_rays_sampled_per_img)
    train_dataset = FullMultiviewDataset(dataset_path=args.dataset_path,
...
def load_trainer(pipeline, train_dataset, diffusion, device, scene_state, args, args_dict) -> BaseTrainer:
    ...
    trainer = SDSTrainer(pipeline=pipeline,
...
diffusion = StableDiffusion(device=device, repo_id=args.repo_id) if args.repo_id else None
trainer = load_trainer(pipeline=pipeline, diffusion=diffusion, train_dataset=train_dataset, device=device, scene_state=scene_state,
